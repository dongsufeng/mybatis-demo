...
@Service(group = "test",version = "1.0.0",timeout = 3000,mock = "com.example.mybatisdemo.service.impl.BookServiceImplMock")
	...
	public AppointExecution appoint(long bookId, long studentId) {
		try {
			...
			if (update <= 0) {// 库存不足
				...
				// 执行预约操作
				int insert = appointmentService.insertAppointment(bookId, studentId);
				if (insert <= 0) {// 重复预约
